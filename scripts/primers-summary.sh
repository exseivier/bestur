...
do
	...
	then
		...
		GENOME_NAME=$(grep "\b$CONTIG_ID\b" $TRANSCRIPTOME_FILE | cut -f1 | sort | uniq | head -n 1)
		TRANS_COORD=$(grep "\b$CONTIG_ID\b" $TRANSCRIPTOME_FILE | cut -f4,5 | sort | uniq | head -n 1)
	fi
