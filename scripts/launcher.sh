					...
					do
						...
						do
							...
							do
								...
								bestur.sh --query $query --db $db --size $size --steps $step --task blastn --evalue $evalue --qcov $q --identity $i --reward $p1 --penalty $p2 --gopen $p3 --gext $p4 --min_frags_per_transfrag $minf --overlap_radius $overlap_r --experiment_name $experiment_name
							done
						done
					done
				done
